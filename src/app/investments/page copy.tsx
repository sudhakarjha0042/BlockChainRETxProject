  ...
  const fetchOwnedProperties = async (address: string, provider: any) => {
    ...
    try {
      ...
      try {
        ...
        console.warn("getUserProperties failed, trying alternative methods:", err);
        
        // Try another approach: Check if the contract has getAllListings to get all properties
        try {
          // Get all available property IDs from the marketplace
          console.log("Trying to get all property listings to check ownership");
          const listings = await contract.getAllListings();
          console.log("All listings:", listings);
          
          // For each property, check if user owns any fractions
          for (const listing of listings) {
            const tokenId = listing.tokenId.toString();
            
            try {
              // Check if user owns fractions of this token
              const ownedFractions = await contract.getUserFractions(address, tokenId);
              
              if (Number(ownedFractions) > 0) {
                console.log(`User owns ${ownedFractions} fractions of token ${tokenId}`);
              ...
              console.warn(`Error checking fractions for token ${tokenId}:`, fractionErr);
            }
          }
          
          if (userProperties.length > 0) {
            console.log("Found user properties via listings check:", userProperties);
          } else {
            console.log("No properties found via listings check");
          }
        } catch (listingErr) {
          console.warn("Could not get all listings:", listingErr);
          
          // Fallback method: Use balanceOf and tokenOfOwnerByIndex (ERC721 standard)
          console.log("Trying balanceOf method as fallback");
          try {
            ...
            console.error("balanceOf method failed:", balanceErr);
          }
        }
      }
      ...
      if (userProperties.length > 0) {
        ...
        console.log("No properties found, checking for properties explicitly by IDs");
        
        // Final attempt: Try to check for specific token IDs starting from 1
        // This is a brute force approach but can help when other methods fail
        try {
          // Try checking the first 10 token IDs
          for (let i = 1; i <= 10; i++) {
            try {
              // Check if this token exists and if user owns fractions
              const property = await contract.properties(i);
              const ownedFractions = await contract.getUserFractions(address, i);
              
              if (Number(ownedFractions) > 0) {
                console.log(`Found ownership: Token ${i}, Fractions: ${ownedFractions}`);
              ...
              // console.warn(`Token ${i} check failed:`, err);
            }
          }
          
          if (userProperties.length > 0) {
            console.log("Found user properties via brute force check:", userProperties);
            setOwnedProperties(userProperties);
            toast({
              title: "Success",
              description: `Found ${userProperties.length} properties in your portfolio`,
            ...
            console.log("No properties found via any method");
            setOwnedProperties([]);
            toast({
          ...
          console.error("Final property check failed:", error);
          setOwnedProperties([]);
          toast({
            title: "No properties found",
            description: "No properties found in your wallet. Try loading sample data for testing.",
