...
export default function Marketplace() {
  ...
  const fetchProperties = React.useCallback(async () => {
    ...
    try {
      ...
      const propertiesFromChain = listings
        .map((listing: BlockchainProperty) => {
          try {
            ...
            // Check for updated availability in localStorage
            const availabilityKey = `property_availability_${listing.tokenId}`;
            const updatedAvailability = localStorage.getItem(availabilityKey);
            const availableFractions =
              updatedAvailability !== null
                ? parseInt(updatedAvailability)
                : Number(listing.availableFractions);
            ...
            return {
              ...
              soldNFTs: Number(listing.totalFractions) - availableFractions,
              location: "Property #" + listing.tokenId.toString(),
              type: "Fractional NFT",
              description: `${availableFractions}/${listing.totalFractions} fractions available`,
              owner: listing.owner,
              availableFractions,
            };
          } catch (err) {
  ...
  const recordPurchase = (propertyId: string, fractions: number) => {
    ...
    const availabilityKey = `property_availability_${propertyId}`;
    const currentAvailability = localStorage.getItem(availabilityKey);
    const updatedAvailability = currentAvailability
      ? parseInt(currentAvailability) - fractions
      : null;

    if (updatedAvailability !== null) {
      localStorage.setItem(availabilityKey, updatedAvailability.toString());
      console.log(`Updated availability for property #${propertyId}: ${updatedAvailability}`);
    }
  };
